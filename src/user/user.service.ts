...
export class UserService {
  ...
  async register(request: RegisterUserRequest): Promise<UserResponse> {
    ...
    const registerUserRequest = this.validationService.validate(UserValidation.REGISTER, request)
    ...
    const user = await this.prismaService.user.create({data: registerUserRequest})
  ...
  async login(request: LoginUserRequest): Promise<UserResponse> {
    ...
    let user = await this.prismaService.user.findUnique({where: {username: loginRequest.username}})
    if (!user) {
  ...
  async update(user: User, request: UpdateUserRequest): Promise<UserResponse> {
    this.logger.debug(`UserService.update(${user}, ${JSON.stringify(request)})`);
