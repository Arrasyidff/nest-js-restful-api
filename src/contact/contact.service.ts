...
export class ContactService {
  ...
  async remove(user: User, contactId: number): Promise<ContactResponse> {
    const contact = await this.checkContactMustExist(user.username, contactId)
    await this.prismaService.contact.delete({where: {username: user.username, id: contactId}})
    return this.toContactResponse(contact)
  }

  async search(user: User, request: SearchContactRequest): Promise<WebResponse<ContactResponse[]>> {
    const searchRequest = this.validationService.validate(ContactValidation.SEARCH, request)
